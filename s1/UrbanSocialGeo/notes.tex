...
\subsection{Classical models of urban form}

\subsubsection{Burgess' land use model, 1925}

Burgess was trying to understand what cities looked like, and what kind of groups can be found.
He found 5 concentric rings: the loop (CBD), zone of transition, zone of working class, suburban zone, commuter zone.
This model is an ideal type, a simplification.

Why five rings? 
\textbf{Bid rent theory}, that different groups of people, are able to pay different rents. The centre has headquarters (can pay higher rents), and further away, suburban, less dense spots (can pay less rent).

It was written in 1925, which is around the time that Chicago was going through an exponential growth. Thus, Burgess makes a \textbf{model of a growing city}, and tries to understand how the growth works in terms of population and the built environment.

The city is characterised by heterogenous population, yet Burgess' model doesn't represent this. Thus, it only really works for Chicago.
...
\begin{outline}
	...
	\1 Commercial and industrial base to the economy of the city
		\2 Other industries can dominate the city, eg. tourism, heritage, religious/pilgrimage
	\1 Private ownership of property and economic competition for space
		\2 Planned economies under communism
	\1 Expanding area and population of the city
		\2 But, cities are marked by shrinkage, eg. Detroit
	\1 Transport is equally easy, rapid, cheap in every direction within the city
	\1 City centre is main centre for employment, thus most valuable
		\2 But, eg. airports are also hubs
	\1 No districts are more attractive because of differences in terrain
		\2 But coasts, mountains, terrain influences attractivity 
	\1 No concentration of heavy industry
	\1 No historic survival of an earlier land-use pattern in any district
\end{outline}

Thus, it's obvious that these assumptions do not apply to any real city, and Burgess acknowledged it. 

The zones are:
\begin{outline}
	\1 \textbf{Loop}: Chicago had an elevated metro track that ran in a loop; place with shopping, offices, HQs, theatres... ie. the functions that are able to pay the highest price for this central location. They need to draw workers/consumers from all corners of the city, thus need to be in the loop
	...
	\1 \textbf{Commuter zone}: better, middle-class residences and commuter belt; 
	\1 
\end{outline}
...
Between 1925-1939, the depression/economic crisis affected the US and less people were willing to move to the US. Thus, Hoyt was thinking of a model where cities grow slowly, and the car plays an increasingly important role.
...
\subsubsection{Harris and Ullman's sector model}
...
The British city, the Chinese city, the African city, LA in 1990s (skid row, gated suburbs), the post-socialist city (Tallinn) 

\subsubsection{Conclusion}

Models have birthmarks, from a specific place and time. They need to be situated as such

\textbf{Provincialisation} - what is it?


\subsection{Patterns of segregation}

Focus on residential segregation, groups that live separately from each other, in different parts of the urban environment. There are five dimensions of segregation: evenness, exposure

\subsubsection{Evenness}

Evenness is the differential distribution of two social groups, among areal units in a city; a minority group is segregated, if it is unevenly distributed over a given area

Index of dissimilarity measures the unevenness in population across space. If an index is low, it means that the population is evenly distributed (ie. less segregation), whereas if an index is 100, the population is unevenly distributed (ie. high segregation). 

\subsubsection{Exposure}

Residential exposure is the possibility of interaction/contact of populations with each other. 

\subsubsection{Concentration}

Amount of physical space occupied by a minority group in the urban. 

\subsubsection{Centralisation}

A degree to which a group is located near the centre of an urban area.

\subsubsection{Clustering}

A degree to which areal units 
...
Why do different population groups take up different parts of the city? There are many threads of geographers, with different opinions

\begin{outline}
	\1 \textbf{Quantitative spatial geography}
	\1 \textbf{Ecological models} of the city, eg. ``urban metabolism'', ``natural forces'', with a pattern of expansion $\rightarrow$ comptition $\rightarrow$ invasion $\rightarrow$ succession
	\1 \textbf{Marxist geography} aims to understand the flow of capital, and the accumulation regime; how segregation fits within the accumulation of capital (Harvey)
	\1 \textbf{Neo-Weberian} approach 
	\1 \textbf{Behavioural geography} based on people's reasoning and preferences around living in a certain environment over another; can done through an ethnic lens;
	\1 \textbf{Post-modern geography}: the meaning of housing, and how this affects one personally; housing is not just structural, but also about feeling at home; for eg., what does moving to the suburbs do to a middle-class family? how does social mobility impact people? why do gentrifiers move back to the city? it's all about a certain identity, and how people understand and distinguish themselves; gentrification is something that's promoted by the cities
\end{outline}
